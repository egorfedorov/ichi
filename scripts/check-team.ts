/**
 * Access-boundary check for shared ichi (migration 0006).
 *
...
import { adoptIchi, setJoinCode, joinByCode, getAccessibleIchi, listAccessibleIchi } from "@/lib/ichi";
...
async function main() {
  ...
  await query(`delete from ichi where owner_id = $1`, [owner]);
  const i = await adoptIchi(owner, "steward", "Repo Keeper");

  const before = await getAccessibleIchi(mate, i.slug);
  ...
  const after = await getAccessibleIchi(mate, i.slug);
  console.log("mate after join       :", after ? "access ✓" : "NO ACCESS (BUG)");

  const str = await getAccessibleIchi(stranger, i.slug);
  console.log("stranger              :", str ? "ACCESS (BUG)" : "no access  ✓");

  const mateList = await listAccessibleIchi(mate);
  ...
  const stillIn = await getAccessibleIchi(mate, i.slug);
